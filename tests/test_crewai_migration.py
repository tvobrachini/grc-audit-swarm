...
from swarm.schema import RiskControlMatrixSchema, QA_PushbackSchema, WorkingPaperSchema, FinalReportSchema
...
def test_planning_phase_schema_guardrail():
    ...
    valid_data = {
        ...
        "risks": [{
            ...
            "controls": [{
                ...
                "testing_procedures": {
                    "test_of_design": [{"step_description": "Check AWS", "expected_result": "Private"}],
                    "test_of_effectiveness": [{"step_description": "Use AWS CLI", "expected_result": "Deny"}]
                }
            }]
        }]
    }
    ...
    invalid_data['risks'][0]['controls'][0]['testing_procedures'].pop('test_of_effectiveness')
    try:
...
def test_fieldwork_phase_evidence_hash_guardrail():
    ...
    result = EvidenceAssuranceProtocol.register_evidence(raw_payload_from_mcp, "aws-s3-mcp.list_buckets")
    ...
    assert EvidenceAssuranceProtocol.verify_exact_quote(vault_id, "internal-fin-data-prod") is True
    assert EvidenceAssuranceProtocol.verify_exact_quote(vault_id, "hallucinated_bucket") is False
