...
def sample_procedure():
    return AuditProcedure(
        control_id="AC-01",
        tod_steps=["Review IAM access policy document.", "Confirm quarterly review requirement is documented."],
        toe_steps=["Extract full population of AD users.", "Sample 25 users and verify access review completion."],
...
def pass_finding():
    return AuditFinding(
        ...
        evidence_extracted=["IAM report shows 325/325 active users reviewed for Q4 2025."],
        risk_rating=None,
        tod_result="Pass",
        toe_result="Pass",
...
def fail_finding():
    return AuditFinding(
        ...
        evidence_extracted=["Tenable scan: CVE-2025-44810 unpatched 18 days.", "No patch ticket created."],
        risk_rating="High",
        tod_result="Pass",
        toe_result="Fail",
...
@pytest.fixture
def state_with_findings(aws_scope_state, pass_finding, fail_finding, exception_finding, sample_control):
