...
st.markdown(
    "This sandbox runs the new IIA Phase 1-3 AI architecture independently to avoid crashing the main app during component refactoring."
)
...
if st.session_state.phase == 0:
    if st.button("🚀 Run Phase 1 (Planning Crew)"):
        ...
        with st.spinner(
            "AI Planning Crew executing (This may take several minutes)..."
        ):
...
if st.session_state.phase >= 2:
    ...
    if st.session_state.phase == 2:
        if st.button(
            "Human Gate 2: Approve Fieldwork & Hashed Evidence (IIA 2340 Stamping)"
        ):
