...
def render_phase1_review(
    ...
    st.info("📋 **Phase 1 Complete** — Review the Risk Control Matrix below, then approve or request changes.")
        ...
        for ctrl in controls:
            ...
            with st.expander(f"**{c_id}** — {str(c_desc)[:80]}…"):
                ...
                with c1:
                    ...
                    for step in tod:
                        st.markdown(f"- {step.get('step_description')} (Expect: {step.get('expected_result')})")
                ...
                # Build excel rows
                rows.append({"Risk ID": r_id, "Control ID": c_id, "Description": c_desc})
    ...
    with col_approve:
        if st.button("🚀 Approve & Target Collection (IIA 2340 Stamping)", type="primary", use_container_width=True):
            on_feedback("approve")
