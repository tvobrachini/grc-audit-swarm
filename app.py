...
# ─── Page Config ──────────────────────────────────────────────────────────────
st.set_page_config(page_title="GRC Audit Swarm", page_icon="🛡️", layout="wide",
...
# ─── Sidebar ──────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown('<div class="hero-badge">🤖 Groq + LangGraph</div>', unsafe_allow_html=True)
    st.title("GRC Audit Swarm")
    st.caption("AI Multi-Agent Audit System")
    st.markdown("---")
    g = os.environ.get("GROQ_API_KEY"); o = os.environ.get("OPENAI_API_KEY")
    if g: st.success("✅ Groq (llama-3.3-70b)")
    elif o: st.warning("⚠️ OpenAI fallback")
    else: st.info("ℹ️ Mock mode")
    st.markdown("---")
...
def lab_files(ext=None):
    if not os.path.exists(LAB_DIR): return []
...
def _get(obj, key, default=""):
    return getattr(obj, key, None) or (obj.get(key, default) if isinstance(obj,dict) else default) or default
...
if not st.session_state.scope_submitted:
    st.markdown("Submit a scope narrative and let the AI swarm research, map controls, execute tests, and present findings for your review.")
    scope_text = ""
    ...
    with colA:
        ...
        if sel != "None" and not scope_text:
            with open(os.path.join(LAB_DIR,sel),"r",encoding="utf-8") as f: scope_text=f.read()
    with colB:
        st.markdown("### 🔍 Preview")
        if scope_text:
            st.text_area("Scope (Editable)", value=scope_text, height=200, key="scope_ta")
    ...
    if c.button("🚀 Launch Swarm", type="primary", use_container_width=True):
        if not scope_text: st.warning("Please provide scope text.")
            ...
            st.session_state.scope_submitted = True
            st.session_state.chat_history.append({"role":"user","content":f"**🚀 {name}** — Scope loaded."})
            st.rerun()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if col_reset.button("🔄 New Audit", use_container_width=True):
        st.session_state.update({"thread_id":str(uuid.uuid4()),"chat_history":[],
                                  "scope_submitted":False,"control_feedback":{}})
        st.rerun()
            ...
            for event in swarm_app.stream(stream_input, config=config, stream_mode="updates"):
                for node, s in event.items():
                    ...
                    if s.get("audit_trail"):
                        ...
                        reasoning = last.get("reasoning_snapshot") if isinstance(last,dict) else getattr(last,"reasoning_snapshot",None)
                    txt = f"🟢 **`{node}`** completed"
                    st.session_state.chat_history.append({"role":"assistant","content":txt,"reasoning":reasoning})
            st.rerun()
    ...
    # ════════════════════════════════════════════════════
        ...
        st.info("📋 **Phase 1 Complete** — Review the planning artifacts below, then approve or give feedback.")
        ...
        with tab2:
            ...
            for ctrl in matrix:
                cid = _get(ctrl,"control_id"); desc = _get(ctrl,"description")
            ...
            if matrix:
                ...
                rows,qrows,erows = [],[],[]
                for ctrl in matrix:
                    cid=_get(ctrl,"control_id");desc=_get(ctrl,"description");procs=_get(ctrl,"procedures")
                    rows.append({"Control ID":cid,"Description":desc})
                    if procs:
                        qrows.append({"Control ID":cid,"TOD Steps":"\n".join(_get(procs,"tod_steps") or [])})
                        erows.append({"ERL":"\n".join(_get(procs,"erl_items") or []),"Control":cid})
                buf = io.BytesIO()
                ...
                st.download_button("📥 Download Audit Plan (Excel)", buf.getvalue(),
                                   "audit_plan.xlsx","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                   use_container_width=True)

        st.markdown("---")
        fb = st.chat_input("Type 'Approve to start execution' or describe what to change...")
        if fb:
            st.session_state.chat_history.append({"role":"user","content":fb})
            if fb.strip().lower() in ["approve","approved","approve to start execution","ok","yes","lgtm","go"]:
    ...
    # ════════════════════════════════════════════════════
        ...
        status_filter = col_f1.selectbox("Filter by Status", ["All","Pass","Exception","Fail"], key="status_filter")
        shown = findings if status_filter=="All" else [f for f in findings if _get(f,"status")==status_filter]
        ...
        st.caption("Review each control, leave targeted feedback, then submit all at the bottom.")
        ...
        for finding in shown:
            ...
            color_class = {"Pass":"finding-pass","Exception":"finding-exception","Fail":"finding-fail"}.get(status,"")

            label = f"{icon} **{cid}** — {status}"
            if risk not in ("N/A", None, ""): label += f" · Risk: **{risk}**"
            ...
            with st.expander(label, expanded=(status in ["Fail","Exception"])):
                ...
                if evids:
                    with st.expander("📎 Evidence Extracted"):
                        for e in evids: st.markdown(f"- {e}")
                ...
                if col_b.button("🚩 Flag as Finding", key=f"flag_{cid}"):
                    swarm_app.update_state(config, {"execution_status": {cid: "flagged"}})
                    st.rerun()
        ...
        with c_sub2:
            if has_pending_feedback:
                st.warning(f"📝 You have notes on {sum(1 for v in st.session_state.control_feedback.values() if v.strip())} control(s). Submit to let the swarm re-evaluate those tests.")
                if st.button("🔁 Submit Feedback & Re-run Flagged Tests", type="primary", use_container_width=True):
                    swarm_app.update_state(config, {
        ...
        if findings:
            ...
            st.download_button("📥 Download Findings Report (Excel)", buf.getvalue(),
                               "audit_findings.xlsx",
    ...
    # ════════════════════════════════════════════════════
        ...
        st.success(f"✅ **Audit Complete!** {passes} controls passed · {fails} controls failed.")
        st.balloons()
